Shader "Fish/BCloud"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			// TODO: make cloud stack together
			#define CLOUD_INIT(uv, sphere_center) \
			...
			v2f vert(appdata v)
			{
				...
				o.uv.zw = (v.uv - 0.5) * float2(_ScaleX, _ScaleY);
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float r = 0.15;
				float repeat_dist = 0.35;
				float2 sphere_center = float2(floor((i.uv.z+r) / repeat_dist) * repeat_dist, 0);
				sphere_center.y = 0.3 * sin(sphere_center.x * 3.16 + 0.233) + 0.1 * sin(sphere_center.x * 4.15 + 2.7);
				float2 uv = i.uv.zw - sphere_center;
				float d2 = dot(uv, uv);
				clip(r * r - d2);
